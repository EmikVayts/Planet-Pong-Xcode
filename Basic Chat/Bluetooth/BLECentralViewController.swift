...
class BLECentralViewController : UIViewController, CBCentralManagerDelegate, CBPeripheralDelegate, UITableViewDelegate, UITableViewDataSource{
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral,advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        //Only add peripheral to the list if it has planet pong in it
        let stringName = String(describing: peripheral.name)
        print(stringName)
        if (stringName.count >= 23) {
            let subStringName = String(stringName.prefix(23))
            print(subStringName)
            if (subStringName == "Optional(\"Planet Pong\")") {
